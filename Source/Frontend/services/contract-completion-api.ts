...
export enum ContractPaymentDueAnchor { ContractSigned = 1, ContractEffectiveDate = 2, AcceptanceCompleted = 3, PreviousMilestonePaid = 4, Manual = 5 }
...
export const contractCompletionApi = {
  ...
  addPayment: (contractId: number, values: { evidenceFile?: File; currentVersionId: number; contractRowVersion: string; versionRowVersion: string; paymentMilestoneId?: number; paymentDate: string; amount: number; currencyCode: string; paymentMethod: string; referenceCode: string }) =>
  ...
  setPaymentMilestoneManualAnchor: (contractId: number, versionId: number, milestoneId: number, values: { currentVersionId: number; contractRowVersion: string; versionRowVersion: string; milestoneRowVersion: string; anchorDate: string; reason: string }) =>
    axiosClient.post<unknown, ContractPaymentMilestoneResponse>(`${BASE_URL}/${contractId}/versions/${versionId}/payment-milestones/${milestoneId}/manual-anchor`, values),
