...
import { getEmployeeTypeLabel } from "@/services/employees-api";

interface ProfileHeroBannerProps {
  profile: EmployeeProfile;
  isReloading?: boolean;
  onReload: () => void | Promise<void>;
  onProfileChanged: (profile: EmployeeProfile) => void;
}
...
export function ProfileHeroBanner({
  ...
  const completion = useMemo(() => {
    const fields = [
      ...
      Boolean(profile.maritalStatus?.trim()),
              ...
              <>
                <Button
                  ...
                  onClick={() =>
                    setPreviewImage({
                      url: coverUrl,
                      title: `Ảnh bìa - ${profile.fullName || "Người dùng"}`,
                      type: "cover",
                    })
                  }
                >
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between -mt-10 sm:-mt-12">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
              <div className="group relative">
                ...
                <div
                  ...
                  onClick={() => {
                    if (avatarUrl) {
                      setPreviewImage({
                        url: avatarUrl,
                        title: `Ảnh đại diện - ${profile.fullName || "Người dùng"}`,
                        type: "avatar",
                      });
                    } else {
                ...
                <div
                  className="absolute -top-1 -left-1 flex size-5 items-center justify-center rounded-full border-2 border-card bg-emerald-500 text-white shadow-sm"
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  ({completion.completedCount}/{completion.totalCount} thông
                  tin)
                </span>
              </div>
              <div className="flex items-center gap-2">
        ...
        <DialogContent className="max-w-3xl overflow-hidden p-0 gap-0 border-border/80 bg-background/95 backdrop-blur-xl shadow-2xl">
          <DialogHeader className="p-4 sm:p-5 border-b border-border/60">
            ...
            <DialogDescription className="text-xs text-muted-foreground">
              Xem ảnh hồ sơ ở kích thước đầy đủ
            </DialogDescription>
          </DialogHeader>
